...
export const StakingDashboard = () => {
  ...
  const { data: deployedTokenData, isLoading: deployedTokenLoading } = useScaffoldContract({
    contractName: "Houdini",
  });

  const { data: deployedStakerData, isLoading: deployedStakerLoading } = useScaffoldContract({
    contractName: "Staker",
  });
  ...
  return (
    <div className="pb-10">
      ...
      <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-2">
        <div className="card card-compact bg-base-100 shadow-xl grid gap-4 sm:grid-cols-1 md:grid-cols-1">
          <div className="card-body">
            <UserStats user={user} apy={apy} earned={earned} timeLeft={timeLeft}></UserStats>
          </div>
        </div>
        <div className="card card-compact bg-base-100 shadow-xl grid gap-4 sm:grid-cols-1 md:grid-cols-1">
          <div className="card-body">
      ...
      <div className="card card-compact bg-base-100 shadow-xl grid gap-4 sm:grid-cols-1 md:grid-cols-1 mt-10">
        <div className="card-body">
          <Buyback token={token} staker={deployedStakerData} approved={approved}></Buyback>
        </div>
      </div>
    </div>
  );
