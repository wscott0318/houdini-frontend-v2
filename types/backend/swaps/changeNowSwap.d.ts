...
export default class ChangeNowSwap implements Swap {
    name: string;
    ...
    getQuote(amount: number, from: string, to: string, fixed: boolean, direction?: QuoteDirection): Promise<any>;
    ...
    swap(amount: number, from: TokenId, to: TokenId, addressTo: string, destinationTag: string, fixed?: boolean, direction?: QuoteDirection, rateId?: string): Promise<SwapResult>;
    status(transactionId: string): Promise<SwapStatus>;
    formatStatus(data: any): SwapStatus;
    formatSwapResult(data: any, extra: any): SwapResult;
}
